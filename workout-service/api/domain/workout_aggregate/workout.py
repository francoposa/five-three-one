...
class Workout(pydantic.BaseModel):
    ...
    def validate_bar_values_count(cls, bar_values: Set[BarValueObject]):
        ...
        if len(bar_values) > cls.MAX_BAR_VALUE_COUNT:
            raise ValueError(
                f"Workout cannot have more than {cls.MAX_BAR_VALUE_COUNT} bar values"
            )
...
class WorkoutList(pydantic.BaseModel):
    class Config:
...
WorkoutList.update_forward_refs()
